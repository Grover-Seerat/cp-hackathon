              ...
              <div className="p-6">
                {tab === "assessment" && (
                  <div>
                    ...
                    <div className="mt-3 rounded border border-hairline bg-ink/40 p-3">
                      <div className="mb-2 flex items-center justify-between">
                        <span className="text-xs uppercase tracking-[0.12em] text-muted">
                          EXIF Data
                        </span>
                        {data.metadata.exif_is_sample && (
                          <span className="rounded bg-stamp/15 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-stamp">
                            Sample — not from this file
                          </span>
                        )}
                      </div>
                      {data.metadata.exif_is_sample && (
                        <p className="mb-2 text-[11px] leading-snug text-muted">
                          This file had no embedded EXIF data — common for images
                          re-saved via WhatsApp, screenshots, or downloads. The
                          fields below are placeholders shown for layout only.
                        </p>
                      )}
                      <dl className="space-y-1 font-mono text-xs">
                        {Object.entries(data.metadata.exif).map(([k, v]) => (
                          <div key={k} className="flex justify-between gap-4">
                            <dt className="text-muted">{k}</dt>
                            <dd className="text-right text-paper/85 break-all">{v}</dd>
                          </div>
                        ))}
                      </dl>
                    </div>
                    ...
                    <ScoreBar
                      label="EXIF fields recovered"
                      value={data.metadata.exif_is_sample ? 0 : Object.keys(data.metadata.exif).length}
                      suffix=""
                      hint="More fields generally indicate less post-processing"
                    />
                    <ScoreBar
                ...
                {tab === "metadata" && (
                  <div className="space-y-1 font-mono text-sm">
                    ...
                    <Row
                      k="EXIF status"
                      v={data.metadata.exif_is_sample ? "No EXIF found in file (sample shown)" : "Extracted from file"}
                    />
                  </div>
                )}
